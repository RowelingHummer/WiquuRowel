...
class navCategorias {
     ...
     */
    public function __construct($nombre,$tamañoArreglo,$arregloImagenes){
        $this->nombreCarrusel=$nombre;
        $this->tamañoCarrusel=$tamañoArreglo;
        $this->array_iconos=$arregloImagenes;
    }
    /**
     * metodo publico que permite la visualizacion del objeto
     */
    public function mostrarCarrusel(){
        echo'<div id="'.$this->nombreCarrusel.'" class="carousel" data-ride="carousel" data-interval="'.$this->time.'" '.$this->detencionHover.'>';
        $this->indicadores();
        $this->envolturas();
        $this->controles();
        echo '</div>';
